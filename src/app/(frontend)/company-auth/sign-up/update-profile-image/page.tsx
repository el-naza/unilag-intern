...
export default function Page() {
  ...
  const updateCompanyImgMtn = useMutation({
  ...
  const form = useForm<{ file?: File }>({
    validators: {
      onSubmitAsync: async ({ value }) => {
        ...
        const error = await updateCompanyImgMtn.mutateAsync(value.file)
        if (error) {
          return {
  ...
  return (
    <div className="text-gray-dark-2 min-h-screen lg:min-h-full py-11 ">
      ...
      >
        ...
        {/* <Button
          ...
          disabled={!uploadedFile || updateCompanyImgMtn.isPending}
          onClick={() => updateCompanyImgMtn.isPending || updateCompanyImgMtn.mutate(uploadedFile!)}
        >
          Proceed to Log in
        </Button> */}
